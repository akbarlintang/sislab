 ...
 */
class Laporanuji extends BaseController
{
    /**
    ...
    public function index()
    {
        ...
        $this->loadViews("laporanuji", $this->global, $data, NULL);
    }
    ...
    function addNew_Form_LHU($row_id)
    {
        ...
        $data['row_id'] = $row_id;
        $data_nofppc = $this->db->query("SELECT no_lhu FROM tbl_fppc WHERE tgl_lhu = CONVERT(date,GETDATE()) ")->result_array();

        if (empty($data_nofppc)) {
            $nosuratlast = $this->db->query("SELECT no_lhu FROM tbl_fppc WHERE tgl_lhu = CONVERT(date,GETDATE()) ORDER BY no_lhu DESC ")->result_array();
            $nobaru = date('d') . '.' . '1';
            $bulan  = $this->getrovbulan(date('m'));
            $tahun  = date('Y');
            $vno_fppc = strval($nobaru) . '/LHU/BALAI KIPM-SMG/' . $bulan . '/' . $tahun;
        } else {
            $nosuratlast = $this->db->query("SELECT no_lhu FROM tbl_fppc WHERE tgl_lhu = CONVERT(date,GETDATE()) ORDER BY no_fppc DESC ")->result_array();
